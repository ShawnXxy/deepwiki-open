...
export async function GET() {
  try {
    const targetUrl = `${TARGET_SERVER_BASE_URL}/filters/config`;
    ...
    const filtersConfig = await backendResponse.json();
    return NextResponse.json(filtersConfig);
  } catch (error) {
    console.error('Error fetching file filters configuration:', error);    
