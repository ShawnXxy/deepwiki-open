...
def _call_llm(
    ...
    *,
    ...
    chat models take ``temperature``. The configured model name is used
    for capability detection because Azure deployment aliases are arbitrary.
...
def generate_wiki(
    ...
    from backend.config import get_azure_ai_client, get_azure_model_name
    structure_model_client = get_azure_ai_client(
        task='premium_reasoning'
    )
    structure_deployment = get_azure_deployment_name(
        task='premium_reasoning'
    )
    structure_model_name = get_azure_model_name(
        task='premium_reasoning'
    )
    page_model_client = get_azure_ai_client(task='reasoning')
    page_deployment = get_azure_deployment_name(task='reasoning')
    page_model_name = get_azure_model_name(task='reasoning')
    ...
    structure_xml, structure_req_id = _call_llm(
        structure_prompt, structure_model_client, structure_deployment,
    ...
    logger.info(
        f"Structure generated by {structure_deployment} "
    ...
    if len(structure_xml) < 200 or '<wiki_structure>' not in structure_xml:
        ...
        structure_xml, structure_req_id = _call_llm(
            structure_prompt, structure_model_client,
        ...
        logger.info(
            f"Structure retry generated by {structure_deployment} "
            f"(req_id={structure_req_id})"
        )
    ...
    for i, page_data in enumerate(pages_data, 1):
        ...
        try:
            content, page_req_id = _call_llm(
                wiki_prompt, page_model_client, page_deployment,
        ...
        if enable_review_pass and not content.startswith('Error'):
            try:
                ...
                reviewed, review_req_id = _call_llm(
                    review_prompt, page_model_client, page_deployment,
                    model_name=page_model_name,
        ...
        logger.info(
            ...
            f"({len(content)} chars, model={page_deployment}, "
